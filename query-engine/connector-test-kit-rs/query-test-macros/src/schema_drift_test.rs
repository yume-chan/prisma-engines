...
pub fn schema_drift_test_impl(attr: TokenStream, input: TokenStream) -> TokenStream {
    let attributes_meta: syn::AttributeArgs = parse_macro_input!(attr as AttributeArgs);
    let args = SchemaDriftArgs::from_list(&attributes_meta);
    let args = match args {
        Ok(args) => args,
    ...
    let handler_a = args.schema_a.unwrap().handler_path;
    let handler_b = args.schema_b.unwrap().handler_path;
    ...
    if test_function.sig.inputs.len() != 2 {
        return syn::Error::new_spanned(
            test_function.sig,
            "Schema drift tests must take exactly two arguments: `runner_a: Runner, runner_b: Runner`.",
        )
        .to_compile_error()
        .into();
    }
    ...
    let test = quote! {
        ...
        fn #test_fn_ident() {
            ...
            if ConnectorTag::should_run(&config, &enabled_connectors, &capabilities, #test_name) {
                let template_a = #handler_a();
                let template_b = #handler_b();

                let datamodel_a = query_tests_setup::render_test_datamodel(config, #test_database, template_a);
                let datamodel_b = query_tests_setup::render_test_datamodel(config, #test_database, template_b);

                let connector = config.test_connector_tag().unwrap();

                query_tests_setup::run_with_tokio(async move {
                    tracing::debug!("Datamodel A:\n {}", datamodel_a.yellow());
                    tracing::debug!("Datamodel B:\n {}", datamodel_b.yellow());

                    query_tests_setup::setup_project(&datamodel_a).await.unwrap();

                    let requires_teardown = connector.requires_teardown();
                    let runner_a = Runner::load(config.runner(), datamodel_a.clone(), connector.clone()).await.unwrap();
                    let runner_b = Runner::load(config.runner(), datamodel_b.clone(), connector).await.unwrap();

                    #runner_fn_ident(runner_a, runner_b).await.unwrap();

                    if requires_teardown { query_tests_setup::teardown_project(&datamodel_a).await.unwrap(); }
